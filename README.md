...
Implement an AI based chat feature where all messages can be broken down and analysed like has been done with the usage examples

### Phrase Library

Similar to the word library I would like to include a library of full phrases that users can get info and quized on in a similar way to words.
...
All JavaScript was tested extensively in a few ways. Firstly, as JS in primarily used to control UI interactions on the page it was tested manually by going through all possible interactions and testing that the JS manipulated or created the UI in the intented way following the test cases outlind below.

Where JS was used to manipulate data via API calls all data being sent and responses being received were printed to the console and analysed to ensure data validity and integrity. All errors are also caught and displayed to the console to catch and fix these errors. Many of the API calls made by the JS don't originate from user entered data to reduce the chance for any invalid data being passed, for example score data where the data generated is generated by the JS on the binary factor of if the user got an anser right or wrong. The data sent using JS was also checked when incoming on the server end by getting Django to print out API requests to the console and checking what had been passed.

Currently only manual unit testing was carried out as I am still having issues with getting the test enviroment to work correctly with based on the tech stack and jquery, as soon as that is resolved automated unit tests will be added.

It is critical when running these JavaScript manual tests that the automated Python/Django tests are run first to ensure they are not they cause of any issues. Many of the JavaScript processes and functions are dependent on received the correct input from Django. If all Django unit tests pass without issue then it can be logically assumed that any failures in the tests below must result from in the JavaScript.

#### Test Case 1: Alert notifications are shown

- **Objective**: Verify that the notification alert is able to be displayed correctly
- **Steps:**
  - Ensure the test alert button is using the correct paramaters needed by the showAlertModal() function (type, message)
  - Click the test flashcard button invoking the the showAlertModal() function
...
- **Steps:**
  - In the final promise of the fetch method used to fetch the word data from the API in word-list.js add a c.log statement to log the JSON object fetched from the API to the console.
  - Navigate to any the word list page of any set that is populated with the word data.
...
- **Expected Results:**
  - An array of JSON object is printed to the console
  - The length of the array is equal to the number of words in the set being accessed.
  - The keys of each JSON object matched the fields of the WORD model in the database
...
- **Expected Results:**
  ...
  - The scores for the current word being accessed matched the scores as reflected in the database.
  - The number of usage examples shown match the number of usage examples created for the current word.
- **Pass/Fail:** Pass if all the expected results occur otherwise fail.

#### Test Case 4: Usage example interactivitiy

- **Objective**: Verify that the usage examples are rendered correctly and when intereracted with display the correct data.
- **Steps:**
  - Click on any word in the word list of any set.
  - Observe the usage examples shown for that word.
  - Hover over every word and observer the results.
  - Click the 'Show Translation' button and observer the results
- **Expected Results:**
  - Each usages example shows the Ukrainian words of the current usage example correctly as one unbroken sentence.
  - When any word is hovered over the correct corresponding romanisation and english word are shown above the word.
  - When the 'Show Translation' button is clicked the text of the button changes the corresponding sentence translation for the current usage example
- **Pass/Fail:** Pass if all the expected results occur otherwise fail.

#### Test Case 4: Word audio for invalid audio

- **Objective**: Verify that the word audio button is displayed and works correclty for a word with valid audio data.
- **Steps:**
  - Veirfy that the word being tested contains a valid audio file link
...
- **Expected Results:**
  ...
  - When the 'Audio' button is clicked the matching pronouncation audio is played.
- **Pass/Fail:** Pass if all the expected results occur otherwise fail.

#### Test Case 5: Word audio for invalid audio

- **Objective**: Verify that the word audio button is not displayed and an alert is shown for words with and invalid audio link.
- **Steps:**
  - Verify that the word being tested contains an invalid audio file link by giving and invalid extentsion or changing the file name to one that does not exist.
  - Open the word details modal for the word being tested.
  - Observer the results
- **Expected Results:**
  - When the word details modal is opened the 'Audio' button is not shown.
  - An alert is shown saying that there was an error loading the pronounciation audio for the current word.
- **Pass/Fail:** Pass if all the expected results occur otherwise fail.

#### Test Case 6: Add word functionality with valid data

- **Objective**: Verify that the add word modal is dispalyed to admin an can successfully add a word with valid data
- **Steps:**
  - Log in as a superuser
  - Naviagte to any word list page
  - Click the 'Add Word' button'
  - Enter valid data into all fields
  - Click the 'Save' button
- **Expected Results:**
  - When the 'Add Word' button is clicked a modal is displayed with fields corrresponding to all fields for the WORD model in the database.
...
- **Steps:**
  ...
  - Naviagte to any word list page
  - Click the 'Add Word' button'
  - Enter invalid data into all fields such a blank fields or an invalid URL or JSON object into relevant fields
  - Click the 'Save' button
- **Expected Results:**
...
- **Steps:**
  ...
  - Click the 'Save' button
- **Expected Results:**
  - When the 'Edit Word' button is click the word edit modal is displayed with a the fields populated with the correct data for the corresponding word being edited.
  - The set checkboxes are automatically checked for the sets that correspond with the word being tested.
  - When the data is saved a alert notification is shown stating that the data has been edited successfully.
  - The edits are reflected in the database.

#### Test Case 9: Edit word functionality with ivalid data
...
- **Objective**: Verify that the delete word functioanlity works correctly by deleting the chosen word from the database
- **Steps:**
  - Log in as a superuser
  - Naviagte to any word list page
  - Click any word on the word list page
...
- **Expected Results:**
  - When the 'Delete' button is clicked an alert is showing stating that the word was successfully deleted from the database
...
- **Steps:**
  ...
  - Adjusted the settings as desired (repeat test using different settings)
...
- **Expected Results:**
  ...
  - Clicking on the 'Start Quiz' button will direct a user to the URL matching the current set and containing paramaters matching the settings that word configured in the modal for quiz type and practice mode.

#### Test Case 12: Flashcard quiz general functioanlity
...
- **Steps:**
  ...
  - Click the 'Correct' button and observer results.
  - Click the 'Flip' button again.
  - Click the 'Incorrect' button and observse results.
...
- **Expected Results:**
  - When be directed to the flashcard quiz page the page is displayed correctly.
  - The intial cards remaining value is equal to double the number of words in the set (two cards for each word).
  ...
  - Also when flip button is clicked the 'Correct' and 'Incorrect' buttons become active and usable.
  - When the 'Correct' or 'Incorrect' buttons are clicked the card flashes the corresponding colour, changes to the next card, the cards remaining is decreased by one and the buttong are set to be inactive again.
  - If the answer or question card is a Ukrainian word the 'Show Romanisation', 'Show Pronounciation' and 'Play Audio' buttons are shown.
  - When the 'Restart' button is clicked the cards remaining is set the original quantity and a valid flashcard is displayed.
  - When the 'Return' button is clicked the user is redirected the the word list for the set they were previously on.

#### Test Case 13: Flashcard quiz practice functioanlity

- **Objective**: Verify that practice specific funtionality of the flashcard quiz works correctly
- **Steps:**
  - Check scores for all words in the set you are about to quiz.
  - Ajust the setting in the quiz modal to be for the flashcard quiz type and practice mode being on.
...
- **Expected Results:**
  ...
  - After the quiz is finished none of the should have been changed.

#### Test Case 14: Flashcard quiz test functioanlity

- **Objective**: Verify that test specific funtionality of the flashcard quiz works correctly
- **Steps:**
  - Check scores for all words in the set you are about to quiz.
  - Ajust the setting in the quiz modal to be for the flashcard quiz type and practice mode being off.
  - Play the quiz choosing 'Correct' for every word
  - Once the quiz is finished recheck all the scores for the words in the set that was tested.
  - Repeat the process using choosing 'Incorrect' for every word.
- **Expected Results:**
  - After the quiz is finished just using correct all the word Flashcard scores should have been incremenented by 20.
  - After the quiz is finished just using incorrect all the word Flashcard scores should have been decremenented by 5.
...
- **Steps:**
  - Ajust the setting in the quiz modal to be for the spelling quiz type
...
- **Expected Results:**
  - After the 'Start' Quiz button is clicked the user is directed to spelling test screen.
  - The number of words remaining is equal to the nunber of words in the current set minus one.
  - When any word in the example sentence is hovered over the romanisation and translation of that word is shown.
  ...
  - When moving to the next card the show translation button is reset and the all the question content is adjusted for the next word in the set.
...
- **Steps:**
  - Ajust the setting in the quiz modal to be for the spelling quiz type and practice mode to be on.
  - Click the 'Start Quiz' button
